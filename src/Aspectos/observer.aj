...
public aspect observer {
	String filepath="src/eventos/buttonActions.txt";
	...
	pointcut receivedUpdate( JPanel jp, Color c) : 
	    ...
	    after(JPanel jp, Color c) :receivedUpdate(jp,c) {
	    	...
	    	String logMessage = String.format("[Auditoria] Se cambió de color %s", LocalDateTime.now());
	...
	pointcut buttonClickEvent( JPanel jp, Color c) : 
	    ...
	    before(JPanel jp, Color c) : buttonClickEvent(jp, c) {
	        String logMessage = String.format("[Registro de Eventos] Botón clickeado a las %s", LocalDateTime.now());
